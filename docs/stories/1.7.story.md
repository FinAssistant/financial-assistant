...
- [x] Implement hardcoded financial keyword detection in orchestrator (AC: 5)
  - [x] Create financial keywords list in `backend/app/ai/financial_keywords.py` as a Python constant (see file for full list)
  - [x] Add keyword detection logic to orchestrator conversation processing
  - [x] Store user messages containing financial keywords in Graphiti via MCP tools
  - [x] Include conversation context (agent type, session info) in episode metadata
- [x] Update orchestrator to use shared Graphiti MCP client (AC: 5, 6)
  - [x] Import and use GraphitiMCPClient in orchestrator agent workflow
  - [x] Call add_episode when financial keywords detected
  - [x] CRITICAL: Use user_id as group_id to ensure user data isolation
  - [x] Include user_id, group_id (same as user_id), message content, and metadata in episodes
  - [x] Ensure agents only access data from group_id matching authenticated user
  - [x] Handle Graphiti storage errors gracefully (log but don't break conversation)
...
- ✅ **TASK 3 COMPLETE**: Create Shared MCP Client for Graphiti Integration
  - ✅ Created backend/mcp_clients/graphiti_client.py with comprehensive GraphitiMCPClient implementation
...
- ✅ **TASK 4 COMPLETE**: Orchestrator Agent Integration with Graphiti
  - ✅ Created comprehensive financial keywords list in backend/financial_keywords.py with 300+ terms across 10 categories
  - ✅ Refactored orchestrator process_message and health_check methods to async for clean Graphiti integration
  - ✅ Implemented financial keyword detection with contains_financial_keywords() function
  - ✅ Integrated GraphitiMCPClient with automatic financial context storage when keywords detected
  - ✅ Added conversation metadata including agent type and session info to Graphiti episodes
  - ✅ Implemented graceful error handling - Graphiti failures don't break conversations
  - ✅ Enhanced health check to include Graphiti MCP server connectivity status
  - ✅ Updated conversation API endpoints to properly handle async orchestrator methods
  - ✅ Added comprehensive test suite with 6 new tests covering keyword detection, storage, error handling, and disconnection scenarios

### File List

- backend/tests/test_graphiti.py (created) - Enhanced Graphiti integration test with multiple episodes and query validation
- backend/mcp_clients/graphiti_client.py (created) - Shared MCP client for Graphiti database operations with user data isolation
...
- backend/app/ai/orchestrator.py (modified) - Added async methods, Graphiti integration, financial keyword detection, and enhanced health check
- backend/app/routers/conversation.py (modified) - Updated to handle async orchestrator methods and added Graphiti health check fields
- backend/tests/test_orchestrator.py (modified) - Updated existing tests to async and added 6 new Graphiti integration tests with mocks
