...
- [ ] **Task 2.1: Fix Backend API Format for AI SDK Compatibility** (AC: 3)
  - [ ] Update `ConversationRequest` model to accept `messages: List[Dict]` instead of single `message`
  - [ ] Replace OpenAI streaming format with AI SDK compatible streaming format
  - [ ] Update endpoint to handle AI SDK `UIMessage[]` format properly
  - [ ] Modify response to use AI SDK expected structure (not OpenAI chat completion chunks)
  - [ ] **Updated Tests**: Fix `backend/tests/test_conversation_endpoints.py` to test AI SDK format
...
- **State Management**: Redux Toolkit 1.9.x with RTK Query for API state management (minimal usage with AI SDK)
...
- **Request Format**: Backend must accept AI SDK's `messages` array format with `UIMessage[]` structure  
...
- **State Tests**: Redux slice testing for conversation state management (minimal with AI SDK)
...
- **Message Format**: Test AI SDK message structures (`UIMessage[]` format)
