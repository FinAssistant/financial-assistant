...
- [ ] Backend: Orchestrator guidance for account connection
  ...
  - [ ] Create token exchange API endpoint for public token → access token conversion
  - [ ] Add Plaid Link token response handling in OnboardingAgent conversation flow
  - [ ] Add system message injection to token exchange endpoint for conversation resumption
  - [ ] Update OnboardingAgent to handle account connection system messages and completion updates
  - [ ] Implement conversation state updates after successful account connection
