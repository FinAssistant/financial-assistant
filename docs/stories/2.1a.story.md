...
- [ ] Backend: Orchestrator guidance for account connection
  - [ ] Add orchestrator logic to prompt users for account connection when profile incomplete
  - [ ] Route to OnboardingAgent for Plaid Link guidance and contextual explanations
  - [ ] Integrate existing plaid_create_link_token MCP tool from Story 1.6 within OnboardingAgent
  - [ ] Implement ConnectedAccount SQL model for secure storage of encrypted Plaid access tokens
  - [ ] Add account CRUD operations in database.py for persistent storage
  - [ ] Update GlobalState with lightweight connected_account_ids for conversation context
