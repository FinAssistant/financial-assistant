...
- [ ] **Dual Storage Integration** (AC: 21, 22, 24)
  ...
  - [ ] Implement SQLite canonical transaction storage with schema and models
  - [ ] Integrate transaction storage service in SpendingAgent workflow
  - [x] Integrate Graphiti via MCP tools for storing spending insights and analysis results
  - [ ] Implement data flow patterns: SQLite (canonical transactions + demographics) → Analysis → Graphiti (store insights)
...
- [ ] **Conversational Interface Integration** (AC: 18, 19, 23)
  - [ ] Replace dummy spending agent in orchestrator with real SpendingAgent subgraph
