...
async def test_financial_entity_extraction():
    ...
    try:
        ...
        required_tools = ["add_memory", "search_memory_nodes", "search_memory_facts"]
        missing_tools = [tool for tool in required_tools if tool not in tool_dict]
        if missing_tools:
            ...
            for query in search_queries:
                ...
                try:
                    ...
                    if isinstance(nodes_response, str) and '"error"' in nodes_response:
                        print(f"❌ Search memory nodes '{query}' failed with API error:")
                    ...
                    print(f"🔍 Search memory nodes '{query}': Found {results_count} results")
                    if results_count > 0:
                ...
                try:
                    ...
                    if isinstance(facts_response, str) and '"error"' in facts_response:
                        print(f"❌ Search memory facts '{query}' failed with API error:")
                    ...
                    print(f"🔍 Search memory facts '{query}': Found {results_count} results")
                    if results_count > 0:
        ...
        try:
            ...
            if isinstance(final_nodes, str) and '"error"' in final_nodes:
                ...
                print(f"🔍 Final memory nodes search found {nodes_count} related contexts")
        ...
        try:
            ...
            if isinstance(final_facts, str) and '"error"' in final_facts:
                ...
                print(f"🔍 Final memory facts search found {facts_count} related contexts")
                total_results += facts_count
            
            if total_results >= 2:  # Should find relationships between memories
                print("🎯 SUCCESS: Graphiti successfully linked financial concepts across memories")
            else:
                print("⚠️  WARNING: Limited context linking detected - may need more processing time")
        ...
        if all_tests_passed:
            ...
            print("⚠️  PARTIAL SUCCESS: Some tests encountered issues but basic functionality works")
